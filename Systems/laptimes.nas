...
var racelap = props.globals.getNode("/Yamaha-YZF/race-lap");
var thissector = props.globals.getNode("/Yamaha-YZF/this-sector");
...
var total_result = func{
	var maxlaps = getprop("/Yamaha-YZF/"~pa~"/race-laps") or 0;
	var tr = 0;
    for(v = 0; v <= maxlaps; v +=1) {
		var actl = getprop("/Yamaha-YZF/"~pa~"/lap["~v~"]/actual-time") or 0;
		tr += actl;
	}
...
var show_lap_and_sector_time = func{
	var show_it = getprop("/Yamaha-YZF/show-race-times") or 0;
	var show_l_or_f = getprop("/Yamaha-YZF/switch-last-fastest") or 0;
	...
	foreach(var s; sectors) { 
		...
		var lsts = getprop("/Yamaha-YZF/"~pa~"/sector["~ln~"]/start-time") or 0;
		var sts  = getprop("/Yamaha-YZF/"~pa~"/sector["~n~"]/start-time") or 0;
		var lt  = getprop("/Yamaha-YZF/"~pa~"/sector["~n~"]/last-time") or 0;
		var ft  = getprop("/Yamaha-YZF/"~pa~"/sector["~n~"]/fastest-time") or 0;
		...
		if(thsec == n and racelap.getValue() > 0){
			...
			if(ct > 0){
				...
				setprop("Yamaha-YZF/this-sector-time-min", result[1]);
			}
			if(ft > 0){
	...
	var fl = getprop("/Yamaha-YZF/"~pa~"/fastest-lap") or 0;
	var ll = getprop("/Yamaha-YZF/last-lap-time") or 0;
	...
	if(fl > 0){
		...
		setprop("Yamaha-YZF/fastest-lap-time-sec", result[0]);
		setprop("Yamaha-YZF/fastest-lap-time-min", result[1]);
		setprop("Yamaha-YZF/fastest-lap-time-hour", result[2]);
	}
	if(lapt > 0){
		var result = calc_time(lapt);
		setprop("Yamaha-YZF/this-lap-time-sec", result[0]);
		setprop("Yamaha-YZF/this-lap-time-min", result[1]);
		setprop("Yamaha-YZF/this-lap-time-hour", result[2]);
	}
	# only for mp
...
var find_marker = func{
	...
	if(getprop("/Yamaha-YZF/"~pa~"/sector["~p~"]/fastest-time") == 0){
	...
	if(wp_distance < 260){
		var courserl = getprop("/Yamaha-YZF/"~pa~"/race-laps") or 0;
	...
	if(wp_distance < 50){
		setprop("/Yamaha-YZF/"~pa~"/sector["~thissector.getValue()~"]/start-time", getprop("/sim/time/elapsed-sec"));
		#print ("Sie ueberfuhren den "~thissector.getValue()~". Wegpunkt "~sectors[thissector.getValue()][3]);
		setprop("/Yamaha-YZF/sector",sectors[thissector.getValue()][3]);

		var fastesttime = getprop("/Yamaha-YZF/"~pa~"/sector["~ln~"]/fastest-time") or 0;
		var lastsectorstarttime = getprop("/Yamaha-YZF/"~pa~"/sector["~ln~"]/start-time") or 0;
		...
		if(thissector.getValue() == 0){
		  ...
		  foreach(var ls; sectors) { 
		  	totallapresult += getprop("/Yamaha-YZF/"~pa~"/sector["~n~"]/last-time") or 0;
			n+=1;
		  }
...
var clear_race_datas = func{
	...
	if(pa != nil and inrange != nil and inrange == 1){
		...
		laptime.setValue(0);
		racetime.setValue(0);
		var act = props.globals.getNode("/Yamaha-YZF/"~pa);
		
		setprop("/Yamaha-YZF/last-lap-time",0);
